...
void Colisionador::CalculeTodasLasFuerzas(Cuerpo * Carga){
	...
	for(i=0;i<N;i++) Carga[i].AgregueFuerza(-Kc*Carga[i].r);
...
int main(void){
	...
	Colisionador Newton;
	double k0=1000, Kcentral=1;
	Newton.Inicie(k0, Kcentral);
...
/*
gnuplot> set title 'POSICION DE UNA PARTICULA COMO FUNCION DEL TIEMPO PARA 5 PERIODOS'
